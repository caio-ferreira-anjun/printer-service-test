...
export class AppService {
  async printFunction(labelObj : LabelDTO) {
    ...
    /*
    // Identificação comentada da Label A
    if(labelObj.label == 'A') {
    ...
    */
    ...
    if( labelObj.label == 'B') {
     const { client, label, container, plp, grid, packages } = labelObj
        ...
        ^FT7,314^A0N,70,71^FB792,1,18,C^FH\^CI28^FD${formatedDate}^FS^CI27
        ^FT0,1142^A0N,54,53^FB749,1,14,R^FH\^CI28^FD${client}^FS^CI27
        ^FT113,507^A0N,56,56^FH\^CI28^FDLabel ${label}^FS^CI27
        ^FT113,694^A0N,56,56^FH\^CI28^FDContainer: ${container}^FS^CI27
        ^FT85,1025^A0N,56,56^FH\^CI28^FDNº da PLP: ${plp}^FS^CI27
        ^FT0,507^A0N,56,56^FB661,1,14,R^FH\^CI28^FDGrid: ${grid}^FS^CI27
    ...
    if (Number(labelObj.grid)>= 30 && Number(labelObj.grid)<= 50) {
      printerName = 'HPRT-HT300-31'
    }
